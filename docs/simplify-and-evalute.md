# 简化与评估 —— 通过简化数据库机制实现架构升级

这是《[记忆的网道](memory-webway.md)》的续篇。网道写成的时候，我还在"真的在考虑使用单一的 sqlite 代替 duckdb+sqlite 的组合"，并列出了四个顾虑：json 的 contains 查询、与 PG 的风格差异、自动化的升级过程、以及换用 sqlite 是否值得。如今这一步已经走完：v0.3.0 发布，六个书架全部运行在新架构上。这篇文档记录这次架构升级的完整推理——为什么简化是正确的，以及它值不值。
