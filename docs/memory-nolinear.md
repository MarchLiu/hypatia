...
粗看起来这不成问题——Agent 框架大多有明确的会话边界，一次启动到一次退出，就是一段对话。但拿这个边界直接当作 session 的定义，整理出来的东西会很难看。一次会话里可能换了三个毫不相干的_topic，而同一个任务又可能横跨好几次会话的重启。整理的单位如果错了，后面所有的摘要和抽取都会对着错误的对象工作。
...
低层原文既然不在热路径上，它也就不必进入向量索引。一个正常使用的书架里，`message` / `session` 这一层往往占到条目总数的一多半，而它们全部被嵌入，会同时付出三笔代价：索引体积与重建时间、召回质量（原始措辞比从中提炼出的知识排得更靠前，恰好短路了逐层下钻的设计），以及写入吞吐（每记录一轮对话都要在 Agent 的热路径上付一次前向计算）。所以 hypatia 允许在写入时就声明不嵌入：书架的 `shelf.toml` 里写
...
这一层的 knowledge 条目就不再产生向量；单条条目也可以自己退出（CLI 的 `--no-embed`，MCP 的 `embed: false`）。statement 不带标签，`skip_tags` 够不着它们，所以 `msg belongTo session` 这类逐轮写入的链接边要自己声明 `--no-embed`——它们是给图遍历用的，没人会按语义去找一条 `belongTo`。两者是同一条规则的两个入口，任一成立就跳过：条目可以比书架更保守，但不能比书架更激进。被跳过的条目照常存储、照常进全文索引与 JSE——按确切措辞找回某句原话是真实有用的，而且代价低廉；退出的只是向量索引。这条规则在写入、自动 flush 和 `backfill` 三处是同一条，所以被跳过的条目不会挂着一笔永远还不掉的嵌入欠债；判定随行存下，被跳过的那一层因此也不会拖慢每条命令都要做的欠债盘点。改 `skip_tags` 对此后写入的条目立即生效，已存的条目由一次 `hypatia backfill` 统一结算。

`summary` 是否也该归入同一档，要按部署自行斟酌：它们确实是提炼过的知识，第 N 层摘要作为语义检索目标很合理；但它们又与同一区间抽出的知识点内容重叠。做成按标签配置，就是为了把这个判断留给部署，而不是由 CLI 硬编码。
...
- **纠错链是金子。** 问题 → 错误答案 → 用户纠正 → 修正，这类片段比一次答对的更有长期价值：它们记录了"为什么错"和"正确的路是什么"，是最不容易从文档里重新获得的knowledge。合成时要保留 initial attempt、错因、正确做法和 lesson 四个部分。
...
这就是三元组的舞台。每个知识点除了正文，还会被拆解为若干 (主语, 谓语, 宾语) 的陈述，写入 RDF 风式的图。正文负责"读到时理解"，三元组负责"需要时找到"——图的遍历是发现隐性关联的唯一手段：从"sqlite"出发沿着关系走两步，你会撞见三年前某次关于 PG jsonb 的讨论，这种相遇在任何全文检索里都不会发生。

我承认三元组的抽取有噪声，谓词的 granularity 也很难完美。但图结构的价值不在于每条边都精确，而在于结构本身提供了可遍历性。一条错误的三元组是一个可被发现、被挑战、被 resolve 的对象；一段没有被结构化的对话，则连犯错的资格都没有。
...
flowchart TB
    ...
    subgraph summaries["对数分层的 Session Summary（log₁₆ n）"]
        direction TB
        L0["L0 原始对话（冷存，不删除）"]
        L1["L1 会话摘要（16:1 归档）"]
        L2["L2 主题摘要（再 16:1）"]
